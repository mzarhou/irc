 ...
 * PRIVMSG COMMAND
 */
PrivMsgCommand::PrivMsgCommand(Context *context, const std::string &name)
    : CmdHandler(context), name(name)
{
}
...
void PrivMsgCommand::run(User &user, const std::string &args)
{
    ...
    oss << user.getMsgPrefix() << " " << this->name << " " << channelTagOrNickname << " " << message << std::endl;
    message = oss.str();
